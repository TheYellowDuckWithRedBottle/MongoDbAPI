...
namespace MongoDB.JWT
{
    public class TokenHelper : ITokenHelper
    {
        ...
        public InToken CreateToken<T>(T user) where T : class
        {
        ...
        public InToken CreateToken(Dictionary<string, string> keyValuePairs)
        {
            List<Claim> claims = new List<Claim>();
            //这里我们通过键值对把数据提供给它
        ...
        private InToken CreateTokenString(List<Claim> claims)
        {
            ...
            return new InToken { TokenStr = new JwtSecurityTokenHandler().WriteToken(token), Expires = expires };
        }
        ...
        {
            var jwtArr = encodeJwt.Split('.');//将jwt分割为三部分
            if (jwtArr.Length < 3)//数据格式都不对直接pass
            {
                return TokenType.Fail;
            }
            var header = JsonConvert.DeserializeObject<Dictionary<string, string>>(Base64UrlEncoder.Decode(jwtArr[0]));//算法和加密类型
            var payLoad = JsonConvert.DeserializeObject<Dictionary<string, string>>(Base64UrlEncoder.Decode(jwtArr[1]));
            var hs256 = new HMACSHA256(Encoding.ASCII.GetBytes(_options.Value.IssuerSigningKey));
            //验证签名是否正确（把用户传递的签名部分取出来和服务器生成的签名匹配即可）
            ...
            //再其次 进行自定义的验证
            if (!validatePayLoad(payLoad))//验证payload中的数据
            {
                return TokenType.Fail;
            }
